...
void solve_simple_iter(double *A, double *x, double *b) {
	...
	auto start = std::chrono::high_resolution_clock::now();
  	int size = n*n;
	int iters = 0;
	#pragma acc data copyin(A[:n*n],Axmb[:n*n],norm_Axmb[:n*n],norm_b)
	{
		do {
			#pragma acc update device(norm_Axmb) async(1)
			#pragma acc parallel loop independent collapse(2) vector vector_length(n) gang num_gangs(n) present(A,Axmb,x,b) 
			...
			norm_Axmb=norm(Axmb);
			next(x, Axmb);
		} while (norm_Axmb/norm_b >= EPS && iters++ < ITERS);
	}
